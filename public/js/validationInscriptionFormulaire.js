document.addEventListener("DOMContentLoaded", function () {
    ...
    function validateName(input, fieldName) {
        ...
        if (!nameRegex.test(value)) {
            showError(
                input,
                errorElement,
                `Le ${fieldName} contient des caractères invalides`,
                "client"
            );
            return false;
        }
    ...
    function getErrorElement(input) {
        return (
            input.nextElementSibling?.querySelector(".error-message") ||
    ...
    function showError(input, errorElement, message, type = "client") {
        ...
        if (errorElement && errorElement.classList.contains("error-message")) {
            ...
            if (
                errorElement.getAttribute("data-type") === "server" &&
                type === "client"
            ) {
                return;
            }
            ...
            if (input.parentElement.nextElementSibling) {
                input.parentElement.parentElement.insertBefore(
                    newErrorElement,
    ...
    function removeError(input, errorElement, type = "client") {
        if (errorElement && errorElement.classList.contains("error-message")) {
            ...
            if (
                errorElement.getAttribute("data-type") === type ||
                type === "all"
            ) {
    ...
    inputs.prenom.addEventListener("blur", () =>
        validateName(inputs.prenom, "prénom")
    );
    ...
    inputs.password_confirmation.addEventListener(
        "blur",
        validatePasswordConfirmation
    );
