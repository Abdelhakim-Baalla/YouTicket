...
function animateDashboardElements() {
    ...
    const cards = document.querySelectorAll(".modern-card, .stat-card-enhanced");
    ...
    ticketItems.forEach(ticket => {
        ticket.addEventListener("click", function(e) {
            ...
            const ticketId = this.dataset.ticketId || 
                            this.querySelector(".ticket-id-modern").textContent.replace('#YT-', '');
...
function updateStatistics(stats) {
    // Tickets assignés
    animateNumber(document.querySelector(".stat-card-enhanced:nth-child(1) .stat-value-enhanced"), 
                 parseInt(document.querySelector(".stat-card-enhanced:nth-child(1) .stat-value-enhanced").textContent.replace(/,/g, "")),
                 stats.assignedTickets, 1000);
    
    // Tickets résolus
    animateNumber(document.querySelector(".stat-card-enhanced:nth-child(2) .stat-value-enhanced"), 
                 parseInt(document.querySelector(".stat-card-enhanced:nth-child(2) .stat-value-enhanced").textContent.replace(/,/g, "")),
                 stats.resolvedTickets, 1000);
    
    // Temps moyen
    document.querySelector(".stat-card-enhanced:nth-child(3) .stat-value-enhanced").textContent = stats.averageTime;
    
    // Satisfaction
    document.querySelector(".stat-card-enhanced:nth-child(4) .stat-value-enhanced").textContent = stats.satisfactionRating;
}
...
function updateTicketList(tickets) {
    ...
    tickets.forEach(ticket => {
        ticketsHTML += `
            <a href="/tickets/${ticket.id}" class="ticket-item-modern" data-ticket-id="${ticket.id}">
                <div class="ticket-priority-bar priority-${ticket.priority}"></div>
                <div class="ticket-header-modern">
                    <div class="ticket-id-modern">#YT-${String(ticket.id).padStart(4, '0')}</div>
                ...
                <div class="ticket-footer-modern">
                    <div class="ticket-client-modern">
                        ...
                        <div class="client-name-modern">${ticket.user.firstName} ${ticket.user.lastName}</div>
                    </div>
                    <div class="ticket-time-modern" data-tippy-content="${new Date(ticket.createdAt).toLocaleString()}">
...
function updateMetrics(metrics) {
    // Taux de résolution
    const resolutionFill = document.querySelector(".metric-item:nth-child(1) .metric-fill");
    if (resolutionFill) {
        resolutionFill.style.width = `${metrics.resolutionRate}%`;
        document.querySelector(".metric-item:nth-child(1) .metric-value").textContent = `${metrics.resolutionRate}%`;
    }
    
    // Tickets en retard
    const overdueFill = document.querySelector(".metric-item:nth-child(2) .metric-fill");
    if (overdueFill) {
        overdueFill.style.width = `${metrics.overduePercentage}%`;
        document.querySelector(".metric-item:nth-child(2) .metric-value").textContent = metrics.overdueCount;
    }
    
    // Satisfaction client
    const satisfactionFill = document.querySelector(".metric-item:nth-child(3) .metric-fill");
    if (satisfactionFill) {
        satisfactionFill.style.width = `${(metrics.satisfactionRating / 5) * 100}%`;
        document.querySelector(".metric-item:nth-child(3) .metric-value").textContent = `${metrics.satisfactionRating}/5`;
    }
}
...
function showNewTicketAssignedNotification(ticket) {
    ...
    notification.innerHTML = `
        ...
        <div class="notification-body">
            <strong>#YT-${String(ticket.id).padStart(4, '0')}</strong> - ${ticket.title}
        </div>
...
function showTicketUpdatedNotification(ticket) {
    ...
    notification.innerHTML = `
        ...
        <div class="notification-body">
            <strong>#YT-${String(ticket.id).padStart(4, '0')}</strong> - Statut: ${getStatusLabel(ticket.status)}
        </div>
        <div class="notification-footer">
            <a href="/tickets/${ticket.id}">Voir le ticket</a>
        </div>
...
function getStatusIcon(status) {
    switch(status) {
        case 'new': return '<i class="fas fa-star"></i>';
        case 'open': return '<i class="fas fa-hourglass-half"></i>';
        case 'pending': return '<i class="fas fa-clock"></i>';
        case 'solved': return '<i class="fas fa-check"></i>';
        case 'closed': return '<i class="fas fa-times"></i>';
