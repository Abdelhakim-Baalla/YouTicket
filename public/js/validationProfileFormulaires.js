...
function validatePersonalInfoForm() {
    ...
    if (telephone.value.trim() && !validatePhone(telephone.value)) {
        showError(
            telephone,
            "Format de téléphone invalide. Exemples: +212612345678, +33123456789, 0123456789"
        );
        isValid = false;
    } else {
    ...
    if (photo.files.length > 0) {
        ...
        if (!validation.type) {
            showError(
                photo,
                "Format de fichier non supporté (JPG, PNG, GIF uniquement)"
            );
            isValid = false;
...
function validateNotificationsForm() {
    ...
    if (!frequency.value) {
        showError(
            frequency,
            "Veuillez sélectionner une fréquence de notification"
        );
        isValid = false;
    } else {
    ...
    if (!isAnyChecked) {
        ...
        let errorElement = notificationSection.querySelector(
            ".notification-error"
        );
        ...
        errorElement.textContent =
            "Veuillez sélectionner au moins un type de notification";
        isValid = false;
    } else {
...
function updatePasswordStrength(password) {
    ...
    strengthElement.textContent = `Force du mot de passe: ${
        messages[score - 1]
    }`;
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const accountSettingsBtn = document.querySelector(
        "#account_settings .btn.btn-primary"
    );
